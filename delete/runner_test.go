...
func TestRun_ByID_Success(t *testing.T) {
	m := &mockAppsService{}
	m.On("Delete", mock.Anything, "app-123").Return(okResp(), nil)
...
func TestRun_DeleteNotFound_Ignore(t *testing.T) {
	m := &mockAppsService{}
	m.On("Delete", mock.Anything, "app-123").Return(notFoundResp(), fmt.Errorf("not found"))
...
func TestRun_DeleteNotFound_Error(t *testing.T) {
	m := &mockAppsService{}
	m.On("Delete", mock.Anything, "app-123").Return(notFoundResp(), fmt.Errorf("not found"))

	r := newTestRunner(t, m, inputs{token: "tok", appID: "app-123", ignoreNotFound: false})
	err := r.run(context.Background())
	assert.ErrorContains(t, err, "failed to delete app")
}

func TestRun_DeleteError(t *testing.T) {
	m := &mockAppsService{}
	m.On("Delete", mock.Anything, "app-123").Return(okResp(), fmt.Errorf("server error"))
...
func TestRun_ByName_Found_DeleteSuccess(t *testing.T) {
	m := &mockAppsService{}
	app := &godo.App{ID: "app-xyz", Spec: &godo.AppSpec{Name: "target-app"}}
	m.On("List", mock.Anything, mock.Anything).Return([]*godo.App{app}, emptyListResp(), nil)
	m.On("Delete", mock.Anything, "app-xyz").Return(okResp(), nil)

	r := newTestRunner(t, m, inputs{token: "tok", appName: "target-app"})
	err := r.run(context.Background())
	assert.NoError(t, err)
	m.AssertExpectations(t)
}
